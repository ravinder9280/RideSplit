...
const RidePin = ({ fromText,toText}: { fromText: string, toText: string }) => {
  return (
      <div className="py-2 flex">
          ...
          <div className="flex-1 space-y-4">
              <div>
                  ...
                  <p className="text-sm font-medium">{fromText}</p>
              </div>

              <div>
                  <p className="text-xs text-muted-foreground">Dropoff Point</p>
                  <p className="text-sm line-clamp-2 font-medium">{toText}</p>
              </div>
          </div>
      </div>  )
}
